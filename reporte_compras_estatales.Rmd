...
distinct(compras, nro_doc_prov)
distinct(oferentes, nro_doc_prov)
...
```{r}
compras %>%
  left_join(select(oferentes, id_compra, nombre_comercial, nro_doc_prov), by = "id_compra") %>%
  group_by(nombre_comercial, nro_doc_prov) %>%
  summarise(total = n_distinct(id_compra)) %>%
  arrange(desc(total)) %>% 
  tail(50)
```
...
compras %>% 
  ...
  ggplot() +
  geom_line(aes(`floor_date(fecha_compra, "week")`, total_comprado)) +
...
compras %>% 
  ...
  tally() %>% 
  ggplot() +
  geom_line(aes(`floor_date(fecha_compra, "week")`, n)) +
...
dim(compras)
dim(distinct(compras, id_compra))

sum(adjudicaciones$precio_tot_imp, na.rm = TRUE) / dim(adjudicaciones)[1]
```
